...
public class HomeController {
	...
	public String home(HttpServletRequest request, Principal principal) {
		...
		if (myAuthorities.contains("ADMIN")){
			...
			if (oLoggedUser.get().getUsertype()==Properties.USER_TYPE_ADMIN) {
		...
		}else if  (myAuthorities.contains("STUDENT")){
			
			Optional<Student> oStudent; 
			
			if (oLoggedUser.get().getUsertype()==Properties.USER_TYPE_STUDENT) {
		...
		}else if (myAuthorities.contains("PROFESSOR")){//  (perfil.equalsIgnoreCase("PROFESSOR"))
			
			Optional<Professor> oProfessor; 
			
			if (oLoggedUser.get().getUsertype()==Properties.USER_TYPE_PROFESSOR) {
